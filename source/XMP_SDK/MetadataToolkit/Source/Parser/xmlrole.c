/* $Header: //xaptk/expat/xmltok/xmlrole.c#2 $ */
/*
...
int prolog0(PROLOG_STATE *state,
        ...
        const ENCODING *enc)
{
  end=end;	/* Avoid unused param warnings. */
  switch (tok) {
  ...
  case XML_TOK_DECL_OPEN:
    if (!SYMACRO(XmlNameMatchesAscii)(enc,
...
int prolog2(PROLOG_STATE *state,
        ...
        const ENCODING *enc)
{
  ptr=ptr; end=end; enc=enc;	/* Avoid unused param warnings. */
  switch (tok) {
  case XML_TOK_PROLOG_S:
    return XML_ROLE_NONE;
  case XML_TOK_PI:
         ...
         const ENCODING *enc)
{
  ...
  case XML_TOK_NAME:
    if (SYMACRO(XmlNameMatchesAscii)(enc, ptr, "SYSTEM")) {
      state->handler = doctype3;
      return XML_ROLE_NONE;
    }
    if (SYMACRO(XmlNameMatchesAscii)(enc, ptr, "PUBLIC")) {
        ...
        const ENCODING *enc)
{
  ...
  case XML_TOK_DECL_CLOSE:
    ...
    return XML_ROLE_NONE;
  case XML_TOK_NAME:
    if (SYMACRO(XmlNameMatchesAscii)(enc, ptr, "NDATA")) {
         ...
         const ENCODING *enc)
{
  ...
  case XML_TOK_NAME:
    ...
    if (SYMACRO(XmlNameMatchesAscii)(enc, ptr, types[i])) {
      state->handler = attlist8;
      return XML_ROLE_ATTRIBUTE_TYPE_CDATA + i;
    }
    }
    if (SYMACRO(XmlNameMatchesAscii)(enc, ptr, "NOTATION")) {
         ...
         const ENCODING *enc)
{
  ...
  case XML_TOK_PROLOG_S:
    return XML_ROLE_NONE;
  case XML_TOK_NAME:
    if (SYMACRO(XmlNameMatchesAscii)(enc, ptr, "EMPTY")) {
      state->handler = declClose;
      return XML_ROLE_CONTENT_EMPTY;
    }
    if (SYMACRO(XmlNameMatchesAscii)(enc, ptr, "ANY")) {
      state->handler = declClose;
...
int error(PROLOG_STATE *state,
       ...
       const ENCODING *enc)
{
  state=state; tok=tok; ptr=ptr; end=end; enc=enc;	/* Avoid unused param warnings. */
  return XML_ROLE_NONE;
}
...
void SYMUNGE(XmlPrologStateInit)(PROLOG_STATE *state)
{
