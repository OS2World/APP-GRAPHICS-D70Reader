/* $Header: //xaptk/expat/xmlparse/hashtable.c#2 $ */
/*
...
NAMED * SYMUNGE(lookup)(HASH_TABLE *table, KEY name, size_t createSize)
{
  ...
  if (table->size == 0) {
    ...
    if (!table->v)
      return 0;
    memset ( table->v, 0, (INIT_SIZE * sizeof(NAMED *)) );	/* ** Not all allocators pre-clear. */
...
void SYMUNGE(hashTableDestroy)(HASH_TABLE *table)
{
  size_t i;
  for (i = 0; i < table->size; i++) {
    NAMED *p = table->v[i];
...
void SYMUNGE(hashTableInit)(HASH_TABLE *p)
{
  p->size = 0;
  p->usedLim = 0;
  p->used = 0;
...
void SYMUNGE(hashTableIterInit)(HASH_TABLE_ITER *iter, const HASH_TABLE *table)
{
  iter->p = table->v;
  iter->end = iter->p + table->size;
}

NAMED * SYMUNGE(hashTableIterNext)(HASH_TABLE_ITER *iter)
{
