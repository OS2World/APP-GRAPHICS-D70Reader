/* $Header: //xaptk/expat/xmltok/xmltok.c#2 $ */
/*
...
{
  enc=enc; p=p;	/* Avoid unused param warnings. */
  return 0;
}
...
{
  enc=enc;	/* Avoid unused param warnings. */
  ...
  if (ptr + 1 == end) {
    ...
    case 0x003C:
      *encPtr = &big2_encoding.enc;
      return SYMACRO(XmlTok)(*encPtr, state, ptr, end, nextTokPtr);
    case 0xFEFF:
      *nextTokPtr = ptr + 2;
      *encPtr = &big2_encoding.enc;
      return XML_TOK_BOM;
...
const ENCODING * SYMUNGE(XmlGetUtf8InternalEncoding)()
{
  return &internal_utf8_encoding.enc;
}

const ENCODING * SYMUNGE(XmlGetUtf16InternalEncoding)()
{
#if BYTE_ORDER == 12
...
int SYMUNGE(XmlInitEncoding)(INIT_ENCODING *p, const ENCODING **encPtr, const char *name)
{
  if (name) {
...
int toAscii(const ENCODING *enc, const char *ptr, const char *end)
{
  char buf[1];
  char *p = buf;
  SYMACRO(XmlUtf8Convert)(enc, &ptr, end, &p, p + 1);
  if (p == buf)
    return -1;
  else
    return buf[0];
...
#define ENCODING_MAX 128
  ...
  char *p = buf;
  int i;
  SYMACRO(XmlUtf8Convert)(enc, &ptr, end, &p, p + ENCODING_MAX - 1);
  if (ptr != end)
    return 0;
  *p = 0;
  for (i = 0; buf[i]; i++) {
  ...
  if (streqci(buf, "UTF-16")) {
    ...
    unsigned stopCCWhining = n; ++stopCCWhining; /* PAC */
...
int SYMUNGE(XmlParseXmlDecl)(int isGeneralTextEntity,
            const ENCODING *enc,
            const char *ptr,
            const char *end,
            const char **badPtr,
            ...
            int *standalone)
{
  ...
  if (!SYMACRO(XmlNameMatchesAscii)(enc, name, "version")) {
  ...
  if (SYMACRO(XmlNameMatchesAscii)(enc, name, "encoding")) {
  ...
  if (!SYMACRO(XmlNameMatchesAscii)(enc, name, "standalone") || isGeneralTextEntity) {
    *badPtr = name;
    return 0;
  }
  if (SYMACRO(XmlNameMatchesAscii)(enc, val, "yes")) {
    if (standalone)
      *standalone = 1;
  }
  else if (SYMACRO(XmlNameMatchesAscii)(enc, val, "no")) {
    if (standalone)
      *standalone = 0;
  }
  else {
...
int SYMUNGE(XmlUtf8Encode)(int c, char *buf)
{
  enum {
...
int SYMUNGE(XmlUtf16Encode)(int charNum, unsigned short *buf)
{
  if (charNum < 0)
    return 0;
  if (charNum < 0x10000) {
...
int SYMUNGE(XmlSizeOfUnknownEncoding)()
{
...
ENCODING *
SYMUNGE(XmlInitUnknownEncoding)(void *mem,
               int *table,
               int (*convert)(void *userData, const char *p),
               void *userData)
{
  ...
  for (i = 0; i < 256; i++) {
    ...
    e->normal.type[i] = BT_OTHER;
      e->utf8[i][0] = (char)SYMUSE(XmlUtf8Encode)(c, e->utf8[i] + 1);
      e->utf16[i] = c;
    }
  }
  e->userData = userData;
