/* $Header: //xaptk/expat/xmlparse/xmlparse.c#4 $ */
/*
...
/* #include "XMPInitTerm.h"		!!! Don't do this, it causes problems with CALLOC & related macros. */

#ifdef XML_UNICODE
#define XML_ENCODE_MAX XML_UTF16_ENCODE_MAX
#define XmlConvert SYMACRO(XmlUtf16Convert)
#define XmlGetInternalEncoding SYMUNGE(XmlGetUtf16InternalEncoding)
#define XmlEncode SYMUNGE(XmlUtf16Encode)
#define MUST_CONVERT(enc, s) (!(enc)->isUtf16 || (((unsigned long)s) & 1))
typedef unsigned short ICHAR;
#else
#define XML_ENCODE_MAX XML_UTF8_ENCODE_MAX
#define XmlConvert SYMACRO(XmlUtf8Convert)
#define XmlGetInternalEncoding SYMUNGE(XmlGetUtf8InternalEncoding)
#define XmlEncode SYMUNGE(XmlUtf8Encode)
...
/* unused : static Processor errorProcessor; */
...
XML_Parser SYMUNGE(XML_ParserCreate)(const XML_Char *encodingName)
{
  XML_Parser parser = MALLOC(sizeof(Parser));
  if (!parser)
    return parser;
  processor = prologInitProcessor;
  SYMUSE(XmlPrologStateInit)(&prologState);
  userData = 0;
  handlerArg = 0;
  ...
  if (!dtdInit(&dtd) || !atts || !dataBuf
    ...
    SYMUSE(XML_ParserFree)(parser);
    return 0;
  }
  dataBufEnd = dataBuf + INIT_DATA_BUF_SIZE;
  SYMUSE(XmlInitEncoding)(&initEncoding, &encoding, 0);
  return parser;
}

XML_Parser SYMUNGE(XML_ExternalEntityParserCreate)(XML_Parser oldParser,
                      ...
                      const XML_Char *encodingName)
{
  ...
  parser = SYMUSE(XML_ParserCreate)(encodingName);
  if (!parser)
    return 0;
...
void SYMUNGE(XML_ParserFree)(XML_Parser parser)
{
  for (;;) {
    TAG *p;
    if (tagStack == 0) {
...
void SYMUNGE(XML_UseParserAsHandlerArg)(XML_Parser parser)
{
  handlerArg = parser;
}

void SYMUNGE(XML_SetUserData)(XML_Parser parser, void *p)
{
...
int SYMUNGE(XML_SetBase)(XML_Parser parser, const XML_Char *p)
{
  if (p) {
...
const XML_Char *SYMUNGE(XML_GetBase)(XML_Parser parser)
{
  return dtd.base;
}

void SYMUNGE(XML_SetElementHandler)(XML_Parser parser,
...
void SYMUNGE(XML_SetCharacterDataHandler)(XML_Parser parser,
                 XML_CharacterDataHandler handler)
{
  characterDataHandler = handler;
}

void SYMUNGE(XML_SetProcessingInstructionHandler)(XML_Parser parser,
                     XML_ProcessingInstructionHandler handler)
{
  processingInstructionHandler = handler;
}

void SYMUNGE(XML_SetDefaultHandler)(XML_Parser parser,
               XML_DefaultHandler handler)
{
  defaultHandler = handler;
}

void SYMUNGE(XML_SetUnparsedEntityDeclHandler)(XML_Parser parser,
                      XML_UnparsedEntityDeclHandler handler)
{
  unparsedEntityDeclHandler = handler;
}

void SYMUNGE(XML_SetNotationDeclHandler)(XML_Parser parser,
                XML_NotationDeclHandler handler)
{
  notationDeclHandler = handler;
}

void SYMUNGE(XML_SetExternalEntityRefHandler)(XML_Parser parser,
                     XML_ExternalEntityRefHandler handler)
{
  externalEntityRefHandler = handler;
}

void SYMUNGE(XML_SetUnknownEncodingHandler)(XML_Parser parser,
...
int XMLPARSEAPI
SYMUNGE(XML_Parse)(XML_Parser parser, const char *s, int len, int isFinal)
{
  if (len == 0) {
    if (!isFinal)
      return 1;
    ...
    SYMACRO(XmlUpdatePosition)(encoding, positionPtr, end, &position);
    nLeftOver = s + len - end;
    if (nLeftOver) {
    ...
    memcpy(SYMUSE(XML_GetBuffer)(parser, len), s, len);
    return SYMUSE(XML_ParseBuffer)(parser, len, isFinal);
  }
}

int SYMUNGE(XML_ParseBuffer)(XML_Parser parser, int len, int isFinal)
{
  ...
  if (errorCode == XML_ERROR_NONE) {
    if (!isFinal)
      SYMACRO(XmlUpdatePosition)(encoding, positionPtr, bufferPtr, &position);
    return 1;
  }
  else {
    eventEndPtr = eventPtr;
...
void *SYMUNGE(XML_GetBuffer)(XML_Parser parser, int len)
{
...
enum XML_Error SYMUNGE(XML_GetErrorCode)(XML_Parser parser)
{
  return errorCode;
}

long SYMUNGE(XML_GetCurrentByteIndex)(XML_Parser parser)
{
  if (eventPtr)
...
int SYMUNGE(XML_GetCurrentLineNumber)(XML_Parser parser)
{
  if (eventPtr) {
...
int SYMUNGE(XML_GetCurrentColumnNumber)(XML_Parser parser)
{
  if (eventPtr) {
    XmlUpdatePosition(encoding, positionPtr, eventPtr, &position);
    positionPtr = eventPtr;
...
void SYMUNGE(XML_DefaultCurrent)(XML_Parser parser)
{
  if (defaultHandler)
    reportDefault(parser, encoding, eventPtr, eventEndPtr);
}

const XML_LChar * SYMUNGE(XML_ErrorString) ( int code  )
{
  ...
  if ( (code > 0) && (code < (int)(sizeof(message)/sizeof(message[0]))) ) return message[code];
...
{
  const char *next;
  int tok = SYMACRO(XmlContentTok)(encoding, start, end, &next);
  switch (tok) {
  case XML_TOK_BOM:
    start = next;
    break;
...
doContent(XML_Parser parser,
      ...
      const char **nextPtr)
{
  const ENCODING *internalEnc = SYMACRO(XmlGetInternalEncoding)();
  const char *dummy;
  const char **eventPP;
  ...
  for (;;) {
    const char *next;
    int tok = SYMACRO(XmlContentTok)(enc, s, end, &next);
    *eventEndPP = next;
    switch (tok) {
    ...
    *nextPtr = s;
    ...
    const XML_Char *name;
    ENTITY *entity;
    XML_Char ch = SYMACRO(XmlPredefinedEntityName)(enc,
    ...
    entity = (ENTITY *)SYMUSE(lookup)(&dtd.generalEntities, name, 0);
    ...
    tag->rawNameLength = SYMACRO(XmlNameLength)(enc, tag->rawName);
    if (nextPtr) {
    ...
    const XML_Char *name = poolStoreString(&tempPool, enc, rawName,
                           rawName
                           + SYMACRO(XmlNameLength)(enc, rawName));
    if (!name)
      return XML_ERROR_NO_MEMORY;
    poolFinish(&tempPool);
    if (startElementHandler) {
    ...
    len = SYMACRO(XmlNameLength)(enc, rawName);
    ...
    case XML_TOK_CHAR_REF:
      {
    int n = SYMACRO(XmlCharRefNumber)(enc, s);
    if (n < 0)
      return XML_ERROR_BAD_CHAR_REF;
    if (characterDataHandler) {
      XML_Char buf[XML_ENCODE_MAX];
      characterDataHandler(handlerArg, buf, SYMACRO(XmlEncode)(n, (ICHAR *)buf));
    }
    else if (defaultHandler)
      reportDefault(parser, enc, s, next);
      }
    ...
    *nextPtr = s;
    ...
    if (MUST_CONVERT(enc, s)) {
      ...
      SYMACRO(XmlConvert)(enc, &s, end, &dataPtr, (ICHAR *)dataBufEnd);
    ...
    *eventPP = end;
    ...
    if (MUST_CONVERT(enc, s)) {
      for (;;) {
        ICHAR *dataPtr = (ICHAR *)dataBuf;
        SYMACRO(XmlConvert)(enc, &s, next, &dataPtr, (ICHAR *)dataBufEnd);
        *eventEndPP = s;
        characterDataHandler(handlerArg, dataBuf, dataPtr - (ICHAR *)dataBuf);
        if (s == next)
          break;
                ...
                const XML_Char *tagName, const char *s)
{
  ...
  if (tagName) {
    elementType = (ELEMENT_TYPE *)SYMUSE(lookup)(&dtd.elementTypes, tagName, 0);
    if (elementType)
      nDefaultAtts = elementType->nDefaultAtts;
  }

  n = SYMACRO(XmlGetAttributes)(enc, s, attsSize, atts);
  if (n + nDefaultAtts > attsSize) {
    ...
    if (!atts)
      return XML_ERROR_NO_MEMORY;
    if (n > oldAttsSize)
      SYMACRO(XmlGetAttributes)(enc, s, n, atts);
  }
  appAtts = (const XML_Char **)atts;
  for (i = 0; i < n; i++) {
    ATTRIBUTE_ID *attId = getAttributeId(parser, enc, atts[i].name,
                      atts[i].name
                      + SYMACRO(XmlNameLength)(enc, atts[i].name));
    if (!attId)
      return XML_ERROR_NO_MEMORY;
    if ((attId->name)[-1]) {
      if (enc == encoding)
                  ...
                  const char **nextPtr)
{
  ...
  *startPtr = 0;
  for (;;) {
    const char *next;
    int tok = SYMACRO(XmlCdataSectionTok)(enc, s, end, &next);
    *eventEndPP = next;
    switch (tok) {
...
#endif
  if (SYMUSE(XmlInitEncoding)(&initEncoding, &encoding, s))
    return XML_ERROR_NONE;
           ...
           const char *s, const char *next)
{
  ...
  const char *version;
  int standalone = -1;
  if (!SYMUSE(XmlParseXmlDecl)(isGeneralTextEntity,
               encoding,
               s,
               next,
               &eventPtr,
  ...
  if (!protocolEncodingName) {
    ...
    return XML_ERROR_INCORRECT_ENCODING;
      ...
      const XML_Char *s = poolStoreString(&tempPool,
                      ...
                      encodingName,
                      encodingName
                      + SYMACRO(XmlNameLength)(encoding, encodingName));
      if (!s)
    return XML_ERROR_NO_MEMORY;
...
handleUnknownEncoding(XML_Parser parser, const XML_Char *encodingName)
{
  if (unknownEncodingHandler) {
    ...
    if (unknownEncodingHandler(unknownEncodingHandlerData, encodingName, &info)) {
      ENCODING *enc;
      unknownEncodingMem = MALLOC(SYMUSE(XmlSizeOfUnknownEncoding)());
      if (!unknownEncodingMem) {
    if (info.release)
      info.release(info.data);
    return XML_ERROR_NO_MEMORY;
      }
      enc = SYMUSE(XmlInitUnknownEncoding)(unknownEncodingMem,
                   info.map,
                   info.convert,
                   info.data);
      if (enc) {
        ...
        const char **nextPtr)
{
  for (;;) {
    const char *next;
    int tok = SYMACRO(XmlPrologTok)(encoding, s, end, &next);
    if (tok <= 0) {
    ...
    *nextPtr = s;
    ...
    switch (SYMACRO(XmlTokenRole)(&prologState, tok, s, next, encoding)) {
    ...
    case XML_ROLE_ENTITY_PUBLIC_ID:
      if (!SYMACRO(XmlIsPublicId)(encoding, s, next, &eventPtr))
    return XML_ERROR_SYNTAX;
      if (declEntity) {
    ...
    if (!name)
      return XML_ERROR_NO_MEMORY;
    declElementType = (ELEMENT_TYPE *)SYMUSE(lookup)(&dtd.elementTypes, name, sizeof(ELEMENT_TYPE));
    if (!declElementType)
      return XML_ERROR_NO_MEMORY;
    ...
    case XML_ROLE_GENERAL_ENTITY_NAME:
      {
    const XML_Char *name;
    if (SYMACRO(XmlPredefinedEntityName)(encoding, s, next)) {
      declEntity = 0;
      break;
    }
    ...
    if (!name)
      return XML_ERROR_NO_MEMORY;
    if (dtd.complete) {
      declEntity = (ENTITY *)SYMUSE(lookup)(&dtd.generalEntities, name, sizeof(ENTITY));
      if (!declEntity)
        return XML_ERROR_NO_MEMORY;
...
enum XML_Error errorProcessor(XML_Parser parser,
                  ...
                  const char **nextPtr)
{
  s=s; end=end; nextPtr=nextPtr;	/* Avoid unused param warnings. */
  return errorCode;
}
#endif
             ...
             STRING_POOL *pool)
{
  ...
  for (;;) {
    const char *next;
    int tok = SYMACRO(XmlAttributeValueTok)(enc, ptr, end, &next);
    switch (tok) {
    case XML_TOK_NONE:
      return XML_ERROR_NONE;
    case XML_TOK_INVALID:
    ...
    XML_Char buf[XML_ENCODE_MAX];
    int i;
    int n = SYMACRO(XmlCharRefNumber)(enc, ptr);
    if (n < 0) {
      if (enc == encoding)
        eventPtr = ptr;
          return XML_ERROR_BAD_CHAR_REF;
    ...
    if (!isCdata
      ...
      break;
    n = SYMACRO(XmlEncode)(n, (ICHAR *)buf);
    if (!n) {
      if (enc == encoding)
        eventPtr = ptr;
      return XML_ERROR_BAD_CHAR_REF;
...
{
  /* unused: const ENCODING *internalEnc = SYMACRO(XmlGetInternalEncoding)(); */
  STRING_POOL *pool = &(dtd.pool);
  entityTextPtr += encoding->minBytesPerChar;
  entityTextEnd -= encoding->minBytesPerChar;
  for (;;) {
    const char *next;
    int tok = SYMACRO(XmlEntityValueTok)(encoding, entityTextPtr, entityTextEnd, &next);
    switch (tok) {
    case XML_TOK_PARAM_ENTITY_REF:
    ...
    XML_Char buf[XML_ENCODE_MAX];
    int i;
    int n = SYMACRO(XmlCharRefNumber)(encoding, entityTextPtr);
    if (n < 0) {
      eventPtr = entityTextPtr;
      return XML_ERROR_BAD_CHAR_REF;
    }
...
reportProcessingInstruction(XML_Parser parser, const ENCODING *enc, const char *start, const char *end)
{
  ...
  tem = start + SYMACRO(XmlNameLength)(enc, start);
  target = poolStoreString(&tempPool, enc, start, tem);
  if (!target)
    return 0;
  poolFinish(&tempPool);
  data = poolStoreString(&tempPool, enc,
            SYMACRO(XmlSkipS)(enc, tem),
...
getAttributeId(XML_Parser parser, const ENCODING *enc, const char *start, const char *end)
{
  ...
  if (!name)
    return 0;
  ++name;
  id = (ATTRIBUTE_ID *)SYMUSE(lookup)(&dtd.attributeIds, name, sizeof(ATTRIBUTE_ID));
  if (!id)
    return 0;
  if (id->name != name)
    poolDiscard(&dtd.pool);
...
const XML_Char *getOpenEntityNames(XML_Parser parser)
{
  ...
  SYMUSE(hashTableIterInit)(&iter, &(dtd.generalEntities));
  for (;;) {
    const XML_Char *s;
    ENTITY *e = (ENTITY *)SYMUSE(hashTableIterNext)(&iter);
    if (!e)
      break;
    if (!e->open)
      continue;
...
int setOpenEntityNames(XML_Parser parser, const XML_Char *openEntityNames)
{
  ...
  while (*openEntityNames != XML_T('\0')) {
    if (*s == XML_T(' ') || *s == XML_T('\0')) {
      ...
      if (!poolAppendChar(&tempPool, XML_T('\0')))
    return 0;
      e = (ENTITY *)SYMUSE(lookup)(&dtd.generalEntities, poolStart(&tempPool), 0);
      if (e)
    e->open = 1;
      if (*s == XML_T(' '))
    s++;
...
{
  poolInit(&(p->pool));
  SYMUSE(hashTableInit)(&(p->generalEntities));
  SYMUSE(hashTableInit)(&(p->elementTypes));
  SYMUSE(hashTableInit)(&(p->attributeIds));
  p->complete = 1;
  p->base = 0;
  return 1;
}
...
{
  HASH_TABLE_ITER iter;
  SYMUSE(hashTableIterInit)(&iter, &(p->elementTypes));
  for (;;) {
    ELEMENT_TYPE *e = (ELEMENT_TYPE *)SYMUSE(hashTableIterNext)(&iter);
    if (!e)
      break;
    if (e->allocDefaultAtts != 0)
      FREE(e->defaultAtts);
  }
  SYMUSE(hashTableDestroy)(&(p->generalEntities));
  SYMUSE(hashTableDestroy)(&(p->elementTypes));
  SYMUSE(hashTableDestroy)(&(p->attributeIds));
  poolDestroy(&(p->pool));
}
...
static int dtdCopy(DTD *newDtd, const DTD *oldDtd)
{
  ...
  SYMUSE(hashTableIterInit)(&iter, &(oldDtd->attributeIds));
  ...
  for (;;) {
    ATTRIBUTE_ID *newA;
    const XML_Char *name;
    const ATTRIBUTE_ID *oldA = (ATTRIBUTE_ID *)SYMUSE(hashTableIterNext)(&iter);
    ...
    if (!name)
      return 0;
    ++name;
    newA = (ATTRIBUTE_ID *)SYMUSE(lookup)(&(newDtd->attributeIds), name, sizeof(ATTRIBUTE_ID));
    if (!newA)
      return 0;
  ...
  SYMUSE(hashTableIterInit)(&iter, &(oldDtd->elementTypes));

  for (;;) {
    int i;
    ELEMENT_TYPE *newE;
    const XML_Char *name;
    const ELEMENT_TYPE *oldE = (ELEMENT_TYPE *)SYMUSE(hashTableIterNext)(&iter);
    if (!oldE)
      break;
    name = poolCopyString(&(newDtd->pool), oldE->name);
    if (!name)
      return 0;
    newE = (ELEMENT_TYPE *)SYMUSE(lookup)(&(newDtd->elementTypes), name, sizeof(ELEMENT_TYPE));
    if (!newE)
      return 0;
    ...
    for (i = 0; i < newE->nDefaultAtts; i++) {
      newE->defaultAtts[i].id = (ATTRIBUTE_ID *)SYMUSE(lookup)(&(newDtd->attributeIds), oldE->defaultAtts[i].id->name, 0);
  ...
  SYMUSE(hashTableIterInit)(&iter, &(oldDtd->generalEntities));

  for (;;) {
    ENTITY *newE;
    const XML_Char *name;
    const ENTITY *oldE = (ENTITY *)SYMUSE(hashTableIterNext)(&iter);
    if (!oldE)
      break;
    name = poolCopyString(&(newDtd->pool), oldE->name);
    if (!name)
      return 0;
    newE = (ENTITY *)SYMUSE(lookup)(&(newDtd->generalEntities), name, sizeof(ENTITY));
    if (!newE)
      return 0;
    if (oldE->systemId) {
             ...
             const char *ptr, const char *end)
{
  if (!pool->ptr && !poolGrow(pool))
    return 0;
  for (;;) {
    SYMACRO(XmlConvert)(enc, &ptr, end, (ICHAR **)&(pool->ptr), (ICHAR *)pool->end);
    if (ptr == end)
      break;
    if (!poolGrow(pool))
      return 0;
...
#if 0	/* Don't need to bother with prototypes for empty routines! */
