...
#ifndef __XMLPacketScanner__
#define __XMLPacketScanner__
...
// =================================================================================================
// The XML::PacketScanner class is used to scan a stream of input for XML packets.  An object is
...
//		- Not fully thread safe, don't make concurrent calls to the same PacketScanner object.
// =================================================================================================
...
public:
	...
	//
	// A newly constructed PacketScanner object has one snip covering the whole input with a state
	// of "not seen".  A block of input that contains a full XML packet is split into 3 parts: a
	// (possibly empty) raw input snip, the packet, and another (possibly empty) raw input snip.  A
	// block of input that contains the start of an XML packet is split into two snips, a (possibly
	...
	XAP_ENUM_TYPE ( SnipState ) {
		eNotSeenSnip,		// This snip has not been seen yet.
		eRawInputSnip,		// This snip is raw input, it doesn't contain any part of an XML packet.
		eValidPacketSnip,	// This snip is a complete, valid XML packet.
		ePartialPacketSnip,	// This snip contains the start of a possible XML packet.
		eBadPacketSnip		// This snip contains a complete, but semantically incorrect XML packet.
	};
	
	XAP_ENUM_TYPE ( CharacterForm ) {	// The values allow easy testing for 16/32 bit and big/little endian.
		eChar8Bit			= 0,
		eChar16BitBig		= 2,
		eChar16BitLittle	= 3,
		eChar32BitBig		= 4,
	...
	#define CharFormIs16Bit(f)			( ((int)(f) & XML::PacketScanner::eChar16BitMask) != 0 )
	#define CharFormIs32Bit(f)			( ((int)(f) & XML::PacketScanner::eChar32BitMask) != 0 )
	
	#define CharFormIsBigEndian(f)		( ((int)(f) & XML::PacketScanner::eCharLittleEndianMask) == 0 )
	#define CharFormIsLittleEndian(f)	( ((int)(f) & XML::PacketScanner::eCharLittleEndianMask) != 0 )
	
	struct SnipInfo {

		SnipInfo ( SnipState state, unsigned long offset, unsigned long length ) :
			fOffset ( offset ),
			fLength ( length ),
			fState ( state ),
			fOutOfOrder ( false ),
		...
		unsigned long	fOffset;		// The byte offset of this snip within the input stream.
		unsigned long	fLength;		// The length in bytes of this snip.
		...
		signed long		fBytesAttr;		// The value of the bytes attribute, -1 if not present.
	...
	PacketScanner ( unsigned long streamLength );
	// Constructs a new PacketScanner object for a stream with the given length.
	...
	void Scan ( const void* bufferOrigin, unsigned long bufferOffset, unsigned long bufferLength );
...
private:	// PacketScanner
	...
	public:
		...
		InternalSnip ( unsigned long offset, unsigned long length );
	...
	public:
		...
		unsigned long	fPacketStart;	// Byte offset relative to the entire stream.
		unsigned long	fPacketLength;	// Length in bytes to the end of the trailer processing instruction.
		signed long		fBytesAttr;		// The value of the bytes attribute, -1 if not present.
		std::string		fEncodingAttr;	// The value of the encoding attribute, if any, with nulls removed.
		CharacterForm	fCharForm;		// How the packet is divided into characters.
		char			fAccess;		// The read-only/read-write access from the end attribute.
		...
		void
		AssociateBuffer ( unsigned long bufferOffset, const void* bufferOrigin, unsigned long bufferLength );
		
		PacketMachine ( unsigned long bufferOffset, const void* bufferOrigin, unsigned long bufferLength );
		~PacketMachine ();
	
	private:	// PacketMachine
	
		XAP_ENUM_TYPE ( RecognizerKind ) {
		...
		unsigned long	fBufferOffset;	// The offset of the data buffer within the input stream.
		...
		void
		SetNextRecognizer ( RecognizerKind nextRecognizer );
	...
	unsigned long		fStreamLength;
	InternalSnipList	fInternalSnips;

	void
	SplitInternalSnip ( InternalSnipIterator snipPos, unsigned long relOffset, unsigned long newLength );
