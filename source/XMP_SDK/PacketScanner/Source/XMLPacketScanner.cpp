...
PacketScanner::PacketMachine::PacketMachine ( unsigned long bufferOffset, const void* bufferOrigin, unsigned long bufferLength ) :
...
PacketScanner::PacketMachine::~PacketMachine ()
{
...
void
PacketScanner::PacketMachine::AssociateBuffer ( unsigned long bufferOffset, const void* bufferOrigin, unsigned long bufferLength )
{
...
void
PacketScanner::PacketMachine::ResetMachine ()
{
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::FindLessThan ( PacketMachine * ths, const char * which )
{
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::MatchString ( PacketMachine * ths, const char * literal )
{
	const int		bytesPerChar	= ths->fBytesPerChar;
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::MatchChar ( PacketMachine * ths, const char * literal )
{
	const int	bytesPerChar	= ths->fBytesPerChar;
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::MatchOpenQuote ( PacketMachine * ths, const char * /* unused */ )
{
	const int	bytesPerChar	= ths->fBytesPerChar;
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::MatchCloseQuote ( PacketMachine * ths, const char * /* unused */ )
{
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::CaptureAttrName ( PacketMachine * ths, const char * /* unused */ )
{
	const int	bytesPerChar	= ths->fBytesPerChar;
	char		currChar;
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::CaptureAttrValue ( PacketMachine * ths, const char * /* unused */ )
{
	const int	bytesPerChar	= ths->fBytesPerChar;
	char		currChar		= 0;
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::RecordStart ( PacketMachine * ths, const char * /* unused */ )
{
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::RecognizeBOM ( PacketMachine * ths, const char * /* unused */ )
{
	const int	bytesPerChar	= ths->fBytesPerChar;
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::RecordHeadAttr ( PacketMachine * ths, const char * /* unused */ )
{
	
	if ( ths->fAttrName == "encoding" ) {
	
		assert ( ths->fEncodingAttr == string() );
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::CaptureAccess ( PacketMachine * ths, const char * /* unused */ )
{
	const int	bytesPerChar	= ths->fBytesPerChar;
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::RecordTailAttr ( PacketMachine * ths, const char * /* unused */ )
{
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::CheckPacketEnd ( PacketMachine * ths, const char * /* unused */ )
{
	const int	bytesPerChar	= ths->fBytesPerChar;

	if ( ths->fPosition == 0 ) {	// First call, decide if there is trailing padding.
	
		const int currLength = (ths->fBufferOffset + (ths->fBufferPtr - ths->fBufferOrigin)) - ths->fPacketStart;
...
// overflow the next time PacketScanner::Scan is called.  This might still be too far, so just keep
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::CheckFinalNulls ( PacketMachine * ths, const char * /* unused */ )
{
...
void
PacketScanner::PacketMachine::SetNextRecognizer ( RecognizerKind nextRecognizer )
{
...
PacketScanner::PacketMachine::TriState
PacketScanner::PacketMachine::FindNextPacket ()
{
...
PacketScanner::InternalSnip::InternalSnip ( unsigned long offset, unsigned long length )
{
...
PacketScanner::InternalSnip::InternalSnip ( const InternalSnip& rhs ) :
	fInfo ( rhs.fInfo ),
	fMachine ( NULL )
{

	assert ( rhs.fMachine.get() == NULL );			// Don't copy a snip with a machine.
	assert ( rhs.fInfo.fEncodingAttr == string() ); // Don't copy a snip with an encoding.
...
PacketScanner::InternalSnip::~InternalSnip ()
{
}	// ~InternalSnip ()
...
static char *	snipStateName [6] = { "not-seen", "raw-data", "good-packet", "xapabout", "partial", "bad-packet" };

void
PacketScanner::DumpSnipList ( const char * title )
{
...
PacketScanner::InternalSnipIterator
PacketScanner::PrevSnip ( InternalSnipIterator snipPos )
{
...
PacketScanner::InternalSnipIterator
PacketScanner::NextSnip ( InternalSnipIterator snipPos )
{
...
PacketScanner::PacketScanner ( unsigned long streamLength ) :
	...
	
{
	...
	// DumpSnipList ( "PacketScanner", fInternalSnips );
...
long
PacketScanner::GetSnipCount ()
{

	// DumpSnipList ( "GetSnipCount", fInternalSnips );
...
bool
PacketScanner::StreamAllScanned ()
{

	// DumpSnipList ( "StreamAllScanned", fInternalSnips );

	InternalSnipIterator	snipPos	= fInternalSnips.begin();

	while ( snipPos != fInternalSnips.end() ) {
		if ( snipPos->fInfo.fState == eNotSeenSnip ) return false;
	}
	return true;
...
void
PacketScanner::SplitInternalSnip ( InternalSnipIterator snipPos, unsigned long relOffset, unsigned long newLength )
{
	...
	if ( relOffset > 0 ) {
		...
		InternalSnipIterator	prevPos	= PrevSnip ( snipPos );		// *** Legit if "snipPos == fInternalSnips.begin()"?
	...
	if ( newLength < snipPos->fInfo.fLength ) {
		...
		const unsigned long		tailLength	= snipPos->fInfo.fLength - newLength;
...
PacketScanner::InternalSnipIterator
PacketScanner::MergeInternalSnips ( InternalSnipIterator firstPos, InternalSnipIterator secondPos )
{
...
void
PacketScanner::Scan ( const void* bufferOrigin, unsigned long bufferOffset, unsigned long bufferLength )
{
	...
	if ( bufferOffset >= fStreamLength ) return;	// *** throw an exception
	if ( bufferLength > (fStreamLength - bufferOffset) ) return;	// *** throw an exception
	if ( bufferOrigin == 0 ) return;	// *** throw an exception
	...
	const unsigned long	endOffset	= bufferOffset + bufferLength - 1;
	InternalSnipIterator	snipPos	= fInternalSnips.begin();
	
	while ( endOffset > (snipPos->fInfo.fOffset + snipPos->fInfo.fLength - 1) ) ++ snipPos;
	if ( snipPos->fInfo.fState != eNotSeenSnip )  return;	// *** throw an exception
	
	relOffset = bufferOffset - snipPos->fInfo.fOffset;
	if ( (relOffset + bufferLength) > snipPos->fInfo.fLength )  return;	// *** throw an exception
	
	SplitInternalSnip ( snipPos, relOffset, bufferLength );		// *** If sequential & prev is partial, just tack on,
	// DumpSnipList ( "Split for buffer", fInternalSnips );
	...
	if ( snipPos->fInfo.fOffset > 0 ) {
		...
		if ( prevPos->fInfo.fState == ePartialPacketSnip ) {
			snipPos = MergeInternalSnips ( prevPos, snipPos );
		}
	...
	while ( ! bufferDone ) {
		...
		if ( foundPacket == PacketMachine::eTriNo ) {
			...
			if ( thisMachine->fPacketStart > snipPos->fInfo.fOffset ) {
				...
				// DumpSnipList ( "Split for data before packet", fInternalSnips );
			}
			...
			if ( foundPacket == PacketMachine::eTriMaybe ) {
				...
				packetSnip->fInfo.fEncodingAttr = thisMachine->fEncodingAttr;
				...
				if ( thisMachine->fPacketLength == snipPos->fInfo.fLength ) {
					...
					snipPos = tailPos;
					// DumpSnipList ( "Split for data after packet", fInternalSnips );
	...
	if ( (snipPos->fInfo.fOffset > 0) && (snipPos->fInfo.fState == eRawInputSnip) ) {
		...
		if ( prevPos->fInfo.fState == eRawInputSnip ) {
			snipPos = MergeInternalSnips ( prevPos, snipPos );
		}
		
	}
	
	// DumpSnipList ( "Scan", fInternalSnips );
...
void
PacketScanner::Report ( SnipInfoVector& snips )
{
	...
	// DumpSnipList ( "Report", fInternalSnips );
